...
LPCWSTR driverName = _T("mv video hook driver2");
...
PCHAR VIDEODRIVER::VideoMemory_GetSharedMemory(void)
{
   ...
   hFile0 = CreateFile(_T("c:\\video0.dat"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
   hFile1 = CreateFile(_T("c:\\video1.dat"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
...
//             MessageBox(NULL,"cxPrimary or cyPrimary not valid", NULL, MB_OK);
        ...
        while (result = EnumDisplayDevices(NULL,
                                  ...
                                  0))
        {
			qDebug()<<QString::fromWCharArray(dispDevice.DeviceString)<<" "<<QString::fromWCharArray(driverName);
          if (wcscmp(&dispDevice.DeviceString[0], driverName) == 0)
              break;
...
//               &dispDevice.DeviceKey[0]);
        ...
        deviceSub = wcsstr(&dispDevice.DeviceKey[0],
                           _T("\\DEVICE"));

        if (!deviceSub) 
            StringCbCopy(&deviceNum[0], sizeof(deviceNum), _T("DEVICE0"));
        else
        ...
        StringCbCopy(&devmode.dmDeviceName[0], sizeof(devmode.dmDeviceName), _T("mv2"));
        deviceName = &dispDevice.DeviceName[0];
...
VIDEODRIVER::Mirror_driver_attach_XP(int x,int y,int w,int h,int depth)
{
    ...
    if (change) 
    {
        ...
        if (!deviceSub) 
            StringCbCopy(&deviceNum[0], MAX_PATH, _T("DEVICE0"));
        else
        ...
        StringCbCopy(&devmode.dmDeviceName[0], 32, _T("mv2"));
        deviceName = &dispDevice.DeviceName[0];
		devmode.dmPelsWidth=w;
		devmode.dmPelsHeight=h;
		devmode.dmPosition.x=x;
		...
		if (pd)
			{
				...
				{
					qDebug()<<QString::fromWCharArray(dd.DeviceString)<<" "<<QString::fromWCharArray(driverName);
					if (wcscmp(&dd.DeviceString[0], driverName) == 0)
					{
					DriverFound=true;
					break;
					}
				...
				if (DriverFound)
				{
				deviceName = &dd.DeviceName[0];
				m_hrootdc = CreateDC(_T("DISPLAY"),deviceName,NULL,NULL);	
